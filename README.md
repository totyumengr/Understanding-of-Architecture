...
如果这个账户系统放到ToC业务或者简单的ToB业务中，我相信OK。但如果是复杂的ToB业务中，就会比较困难，比如ToB业务的服务对象是企业，而企业的组织架构怎么表达，组织架构内不同部门间的数据权限如何管控，业绩如何迁移等等。所以在我看来所谓建模就是如何优雅的表述和处理对象间的关系，且这里面重要的一点就是```先抛离系统```，从事实出发进行业务抽象。组织是一直存在的客观对象，客户是它在某个系统内的概念，而组织并不一定会使用这个系统。
