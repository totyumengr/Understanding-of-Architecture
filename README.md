...
这是个好问题（虽然最后没有去这家公司），值得深思。希望以下的一些观点、结论能够```部分回答```这个问题。同时也借此描绘出架构师们的内心世界，给仍在这条路上前行的人一丝力量。
...
#### 怎么做架构
1. 和当初在学面向对象设计时一样，笔者的理解是多观察现实世界的一些事物，比如线程池的设计和平时咱们去银行办业务时被接待的场景是很像的。
...
2. 支持多租户的系统一定围绕其核心资源搭建一个管理模块形成Kernel，向外暴露API接口和开发工具包，再联合一些支撑型组件形成”OS or Infrastructure“。拿广告业务举例，这个核心资源就是流量资源，广告主都是在抢夺能曝光的机会而已。管理模块就是Ad-Server(广义的，包括算法)，由它来确保对流量资源的安全&高效实用。”OS“层就包括了账户、财务、物料、人群、策略数据等组件。这三层事物加起来，就形成了一个多租户基础设施。
